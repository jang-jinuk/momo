...
//                              @RequestParam(value = "page", defaultValue = "1") int page,
//                              @RequestParam(value = "query", defaultValue = "") String query) {
//  log.info(".......... [GET /manage-user]..........KSW");
//  // 검색어를 사용하여 리포트 조회
//  List<UserDTO> findUsers;
//  if (query.isEmpty()) {
//    findUsers = userService.readALLUsers(); // 검색어가 비었으면 모든 사용자 조회
//  } else {
//    findUsers = userService.searchUsers(query); // 검색어에 따른 사용자 조회
//  }
//
//  //페이지 수 설정
//  int totalUsers = findUsers.size(); // 총 데이터 수
//  int pageSize = 10; // 한 번에 표시할 페이지 수
//  int lastPage = (totalUsers + pageSize - 1) / pageSize; // 총 페이지 수
//
//  // 페이지에 맞게 데이터 나누기
//  int fromIndex = (page - 1) * pageSize;
...
//  model.addAttribute("startPage", startPage);
//  model.addAttribute("endPage", endPage);
//  log.trace("...... [AdminController/manage-User/ END]..........KSW");
//  return "admin/manage-User";
//}

//제제
//@PostMapping("/deactivate-user")
//public String deactivateUserPost(@RequestParam("UserNo") Long UserNo,
//                                 @RequestParam("currentPage") int currentPage,
//                                 @RequestParam("query") String query) throws UnsupportedEncodingException {
//  log.info("...... [POST /deactivate-user]..........KSW");
//  UserDTO UserDTO = new UserDTO(); // 타입에 맞게 객체를 생성하여
//  UserDTO.setUserNo(UserNo); //담아주고
//  // UserService.updateUser() 메서드 호출
//  UserService.deactivateUser(UserDTO); //기능으로 넘겨준다
//  log.info("...... [정의구현]..........KSW");
//  log.info("...... [{}]..........KSW",query);
//  String encodedQuery = URLEncoder.encode(query, StandardCharsets.UTF_8); //쿼리를 인코더에 담아준다
...
////제제해제
//@PostMapping("/reactivate-user")
//public String reactivateUserPost(@RequestParam("UserNo") Long UserNo,
//                                 @RequestParam("currentPage") int currentPage,
//                                 @RequestParam("query") String query) throws UnsupportedEncodingException {
//  log.info("...... [GET /reactivate-user]..........KSW");
//  UserDTO UserDTO = new UserDTO(); // 타입에 맞게 객체를 생성하여
//  UserDTO.setUserNo(UserNo); //담아주고
//  UserService.reactivateUser(UserDTO); //기능으로 넘겨준다
//  log.info("...... [공소시효 만료]..........KSW");
//  String encodedQuery = URLEncoder.encode(query, "UTF-8"); //한글깨짐 방지
//  return "redirect:/admin/manage-User?page=" + currentPage + "&query=" + encodedQuery;
//}
